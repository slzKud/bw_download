...
if(isset($order_column)){
    ...
    switch($i){
        ...
        case 3;$orderSql = " order by username ".$order_dir;break;
        case 4;$orderSql = " order by cardtype ".$order_dir;break;
        case 5;$orderSql = " order by date ".$order_dir;break;
        case 6;$orderSql = " order by result ".$order_dir;break;
        default;$orderSql = '';
    }
}
//搜索
...
//定义查询数据总记录数sql
$sumSql = "SELECT count(id) as sum FROM bw_cardhistory";
//条件过滤后记录数 必要
$recordsFiltered = 0;
//表的总记录数 必要
$recordsTotal = 0;
...
//定义过滤条件查询过滤后的记录数sql
$sumSqlWhere =" where cardid LIKE '%".$search."%'"." or username||result LIKE '%".$search."%'";
if(strlen($search)>0){
...
//query data
$totalResultSql = "SELECT time as 充值时间,id as ID,cardid as 卡号,cardtype as 卡片类型,username as 充值用户名,result as 结果 FROM bw_cardhistory";
$infos = array();
if(strlen($search)>0){
    //如果有搜索条件，按条件过滤找出记录
    ...
    while ($row = mysqli_fetch_array($dataResult)) {
        $obj = array($row['ID'],$row['充值时间'], $row['卡号'], $row['卡片类型'], $row['充值用户名'], $row['结果']);
