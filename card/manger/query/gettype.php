...
if(isset($order_column)){
    ...
    switch($i){
        ...
        case 1;$orderSql = " order by fname ".$order_dir;break;
        case 2;$orderSql = " order by modname ".$order_dir;break;
        default;$orderSql = '';
    }
}
//搜索
...
//定义查询数据总记录数sql
$sumSql = "SELECT count(id) as sum FROM bw_cardtype";
//条件过滤后记录数 必要
$recordsFiltered = 0;
//表的总记录数 必要
$recordsTotal = 0;
...
//定义过滤条件查询过滤后的记录数sql
$sumSqlWhere =" where fname LIKE '%".$search."%' ";
if(strlen($search)>0){
...
//query data
$totalResultSql = "SELECT id,fname,modname FROM bw_cardtype";
$infos = array();
if(strlen($search)>0){
    //如果有搜索条件，按条件过滤找出记录
    ...
    while ($row = mysqli_fetch_array($dataResult)) {
        $obj = array($row['id'],$row['fname'], $row['modname']);
